...
- **Where to find:**
  ...
  - **CLI:** Run `catalyst project:list` — shows a table of project names and IDs.
  - **File:** `catalyst.json` at project root (field `project_id`).
- **When you need it:** API calls, CLI operations, constructing function invocation URLs.
- **Format:** Numeric string, e.g. `"123456789"`.
- **Important:** Do NOT modify `project_id` in `catalyst.json` manually. Use `catalyst project:use`
  to switch projects.

```json
// catalyst.json (auto-generated by `catalyst init`)
{
...
const signupConfig = {
  platform_type: 'web',
  zaid: 10014774358  // ← Get from Settings → Environments
};
const userConfig = {
  last_name: 'Burrows',
...
- **Note:** File Store is deprecated (EOL April 30, 2026). Use Stratus for new projects.
...
<script src="https://static.zohocdn.com/catalyst/sdk/convokraft.js"></script>
<script>
...
const userConfig = {
  last_name: 'Burrows',
  email_id: 'emma@example.com',
  zaaid: 10014774358  // ← Org ID of existing organization
};
...
|---|---|---|---|
| **Project ID** | Project identifier | Settings → General; `catalyst.json`; `catalyst project:list` | Numeric string |
